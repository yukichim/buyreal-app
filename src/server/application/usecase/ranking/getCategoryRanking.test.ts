...
describe('GetCategoryRankingUseCaseInteractor', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    useCase = new GetCategoryRankingUseCaseInteractor(mockCategoryRankingRepository);
  });
  ...
  describe('execute', () => {
    it('should return category rankings from repository with correct limit', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(mockCategoryRankings);

      const result = await useCase.execute(inputData);

      expect(result).toEqual(mockCategoryRankings);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledTimes(1);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(5);
    });

    it('should return empty array when no categories found', async () => {
      const inputData = new GetCategoryRankingUseCaseInputData(10);
      
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce([]);

      const result = await useCase.execute(inputData);

      expect(result).toEqual([]);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledTimes(1);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(10);
    });
    ...
    it('should work with limit 1', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(singleCategory);

      const result = await useCase.execute(inputData);

      expect(result).toEqual(singleCategory);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(1);
    });
    ...
    it('should work with large limits', async () => {
      ...
      expect(result).toEqual(mockCategoryRankings);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(1000);
    });
    ...
    it('should handle repository errors', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockRejectedValueOnce(error);
    ...
    it('should handle different limit values correctly', async () => {
      ...
      for (const limit of testCases) {
        ...
        const inputData = new GetCategoryRankingUseCaseInputData(limit);
        const expectedResults = mockCategoryRankings.slice(0, Math.min(limit, mockCategoryRankings.length));
        
        vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(expectedResults);

        const result = await useCase.execute(inputData);

        expect(result).toEqual(expectedResults);
        expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(limit);
      }
    });
    ...
    it('should handle multiple consecutive calls', async () => {
      ...
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledTimes(2);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenNthCalledWith(1, 2);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenNthCalledWith(2, 5);
    });

    it('should handle null/undefined repository responses gracefully', async () => {
      const inputData = new GetCategoryRankingUseCaseInputData(5);
      
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(null as any);
    ...
    it('should work with decimal limits', async () => {
      ...
      expect(result).toEqual(mockCategoryRankings);
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(3.5);
    });
    ...
    it('should preserve ranking order from repository', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(orderedRankings);

      const result = await useCase.execute(inputData);

      expect(result).toEqual(orderedRankings);
      expect(result).not.toEqual(mockCategoryRankings); // 元の順序と異なることを確認
      expect(result[0].Rank).toBe(3); // 最初の要素が元々3位だったものであることを確認
    });
    ...
    it('should handle categories with different data structures', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(diverseCategories);
    ...
    it('should handle edge case with very large product counts and sales', async () => {
      ...
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValueOnce(largeCategories);

      const result = await useCase.execute(inputData);

      expect(result).toEqual(largeCategories);
      expect(result[0].ProductCount).toBe(Number.MAX_SAFE_INTEGER);
      expect(result[0].TotalSales).toBe(Number.MAX_SAFE_INTEGER);
    });
  });
  ...
  describe('input validation integration', () => {
    it('should fail when trying to create invalid input data', async () => {
      ...
      expect(mockCategoryRankingRepository.getTopCategories).not.toHaveBeenCalled();
    });
  ...
  describe('repository integration', () => {
    it('should only call getTopCategories method on repository', async () => {
      ...
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledTimes(1);
      expect(mockCategoryRankingRepository.updateCategoryStats).not.toHaveBeenCalled();
      expect(mockCategoryRankingRepository.getCategoryStats).not.toHaveBeenCalled();
    });
  ...
  describe('performance considerations', () => {
    it('should handle requests for very large limits efficiently', async () => {
      ...
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledWith(10000);
    });

    it('should handle multiple concurrent requests', async () => {
      const inputData = new GetCategoryRankingUseCaseInputData(5);
      
      vi.mocked(mockCategoryRankingRepository.getTopCategories).mockResolvedValue(mockCategoryRankings);

      const promises = Array.from({ length: 10 }, () => useCase.execute(inputData));
      ...
      expect(mockCategoryRankingRepository.getTopCategories).toHaveBeenCalledTimes(10);
    });
  });
});
