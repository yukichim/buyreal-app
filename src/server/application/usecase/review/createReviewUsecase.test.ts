...
describe("CreateReviewUseCaseInteractor", () => {
	...
	describe("execute", () => {
		it("should create and save a valid review", async () => {
			...
			expect(result.getId().value).toBe("mock-review-id-123");
			expect(result.getProductId()).toBe("product-1");
			expect(result.getBuyerId()).toBe("buyer-1");
			expect(result.getSellerId()).toBe("seller-1");
			expect(result.getRating()).toBe(5);
			expect(result.getComment()).toBe("Excellent product!");
			expect(result.getProductTitle()).toBe("Test Product");
			expect(result.getBuyerName()).toBe("Test Buyer");
		...
		it("should accept all valid ratings (1-5)", async () => {
			...
			for (const rating of validRatings) {
				...
				expect(result.getRating()).toBe(rating);
				expect(mockReviewRepository.save).toHaveBeenCalledTimes(1);
			}
		});
		...
		it("should handle long comment", async () => {
			...
			expect(result.getComment()).toBe(longComment);
			expect(mockReviewRepository.save).toHaveBeenCalledTimes(1);
		});
		...
		it("should handle special characters in comment", async () => {
			...
			expect(result.getComment()).toBe(specialComment);
			expect(mockReviewRepository.save).toHaveBeenCalledTimes(1);
		});
		...
		it("should generate unique IDs for different reviews", async () => {
			...
			expect(results[0]?.getId().value).toBe("id-1");
			expect(results[1]?.getId().value).toBe("id-2");
			expect(results[2]?.getId().value).toBe("id-3");
	...
	describe("repository integration", () => {
		it("should only call save method on repository", async () => {
			...
			expect(mockReviewRepository.findById).not.toHaveBeenCalled();
		});
	});
});
