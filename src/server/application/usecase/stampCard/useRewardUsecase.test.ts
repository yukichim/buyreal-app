...
const createMockStampCard = (
  ...
  addStamp: vi.fn(),
  canGetReward: vi.fn().mockReturnValue(canGetReward),
  useReward: vi.fn(),
}) as StampCardEntity;
...
describe('UseRewardUseCaseInteractor', () => {
  ...
  describe('execute', () => {
    ...
    it('should handle repository save error', async () => {
      ...
      await expect(useCase.execute(inputData)).rejects.toThrow('Failed to save stamp card');
    ...
    it('should fail for different users who have insufficient stamps', async () => {
      ...
      for (const { userId, stamps } of testCases) {
        ...
        await expect(useCase.execute(inputData)).rejects.toThrow('スタンプが足りません');
    ...
    it('should handle multiple consecutive reward uses for same user', async () => {
      ...
      expect(stampCard1.useReward).toHaveBeenCalledTimes(1);
      expect(stampCard2.useReward).toHaveBeenCalledTimes(1);
